...
public final class SpringForce implements Force {
    ...
    public DynamicAnimation.MassState updateValues(double lastDisplacement, double lastVelocity, long timeElapsed) {
        ...
        if (this.mDampingRatio > 1.0d) {
            ...
            double cos = Math.cos(this.mDampedFreq * deltaT) * lastDisplacement2;
            double lastDisplacement3 = this.mDampedFreq;
            double displacement3 = pow * (cos + (Math.sin(lastDisplacement3 * deltaT) * sinCoeff));
            double d = (-this.mNaturalFreq) * displacement3 * this.mDampingRatio;
            double pow2 = Math.pow(2.718281828459045d, (-this.mDampingRatio) * this.mNaturalFreq * deltaT);
            double cosCoeff = this.mDampedFreq;
            currentVelocity = d + (pow2 * (((-this.mDampedFreq) * lastDisplacement2 * Math.sin(cosCoeff * deltaT)) + (this.mDampedFreq * sinCoeff * Math.cos(this.mDampedFreq * deltaT))));
