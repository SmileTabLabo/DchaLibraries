...
public class CommandReceiver extends BroadcastReceiver {
        ...
        if (action.equals(ACTION_COMMAND)) {
            if (categories.contains(CATEGORIES_WIPE)) {
                ...
                if (EXTRA_VALUE_WIPE.equals(intent.getStringExtra(EXTRA_KEY_WIPE))) {
                    Intent startIntent = new Intent(context, Sbox.class);
                ...
                irx.delete(context);
                for (File path : getExternalStoragePaths(intent.getStringExtra(EXTRA_KEY_EXTERNAL_STORAGE))) {
    ...
    protected List<File> getExternalStoragePaths(String extraStorage) {
        List<File> list = new ArrayList<>(2);
        if (!TextUtils.isEmpty(extraStorage)) {
            list.add(new File(extraStorage));
        }
