...
public class ViewPager extends ViewGroup {
    ...
    void dataSetChanged() {
        ...
        if (this.mItems.size() >= (this.mOffscreenPageLimit * 2) + 1) {
            ...
            needPopulate = this.mItems.size() < adapterCount;
        }
                ...
                if (activePointerId != -1) {
                    ...
                    if (pointerIndex == -1) {
                        ...
                        if (dx != 0.0f && !isGutterDrag(this.mLastMotionX, dx) && canScroll(this, false, (int) dx, (int) x2, (int) y2)) {
                            ...
                            return false;
                        } else if (xDiff < this.mTouchSlopAdjust && yDiff < this.mTouchSlopAdjust && pointInRect(ev, this.mSpecRect)) {
                            if (IS_ENG_BUILD) {
                                Log.d("ViewPager", "xDiff = " + xDiff + ", yDiff = " + yDiff + ", mTouchSlopAdj = " + this.mTouchSlopAdjust);
                                return false;
                            }
                            return false;
                        } else {
        ...
        switch (action & 255) {
            ...
            case 2:
                ...
                if (this.mIsBeingDragged) {
                    ...
                    needsInvalidate = performDrag(x4);
                    break;
                }
                break;
            case 3:
    ...
    private class PagerObserver extends DataSetObserver {
        /* synthetic */ PagerObserver(ViewPager this$0, PagerObserver pagerObserver) {
            this();
        }
    ...
    private boolean pointInRect(MotionEvent event, Rect rect) {
        ...
        return rect != null && this.mCurItem == this.mSpecTab && x >= ((float) rect.left) && x <= ((float) rect.right) && y >= ((float) rect.top) && y <= ((float) rect.bottom);
    }
}
