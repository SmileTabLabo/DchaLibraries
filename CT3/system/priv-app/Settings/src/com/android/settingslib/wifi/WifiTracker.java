...
public class WifiTracker {
            ...
            if (configs != null) {
                ...
                for (WifiConfiguration config2 : configs) {
                    if (!config2.selfAdded || config2.numAssociation != 0) {
                        AccessPoint accessPoint2 = getCachedOrCreate(config2, cachedAccessPoints);
                            ...
                            if (!config2.isPasspoint() || this.mIncludePasspoints) {
                                ...
                                while (true) {
                                    if (result$iterator.hasNext()) {
                                        if (((ScanResult) result$iterator.next()).SSID.equals(accessPoint2.getSsidStr())) {
                                            apFound = true;
                                            break;
                                        }
                                    } else {
            ...
            if (results != null) {
                for (ScanResult result : results) {
                    if (result.SSID != null && result.SSID.length() != 0 && !result.capabilities.contains("[IBSS]")) {
                        boolean found = false;
                        Iterator accessPoint$iterator = apMap.getAll(result.SSID).iterator();
                        while (true) {
                            if (accessPoint$iterator.hasNext()) {
                                if (((AccessPoint) accessPoint$iterator.next()).update(result)) {
                                    found = true;
                                    break;
                                }
                            } else {
                        ...
                        if (!found && this.mIncludeScans) {
                            AccessPoint accessPoint3 = getCachedOrCreate(result, cachedAccessPoints);
                            if (this.mLastInfo != null && this.mLastNetworkInfo != null) {
                                accessPoint3.update(connectionConfig, this.mLastInfo, this.mLastNetworkInfo);
                            }
                            if (result.isPasspointNetwork() && (config = this.mWifiManager.getMatchingWifiConfig(result)) != null) {
                                accessPoint3.update(config);
                            }
                            if (this.mLastInfo != null && this.mLastInfo.getBSSID() != null && this.mLastInfo.getBSSID().equals(result.BSSID) && connectionConfig != null && connectionConfig.isPasspoint()) {
                                accessPoint3.update(connectionConfig);
                            }
                            accessPoints.add(accessPoint3);
                            apMap.put(accessPoint3.getSsidStr(), accessPoint3);
                        }
                    }
                }
            }
    ...
    private final class WifiTrackerNetworkCallback extends ConnectivityManager.NetworkCallback {
        /* synthetic */ WifiTrackerNetworkCallback(WifiTracker this$0, WifiTrackerNetworkCallback wifiTrackerNetworkCallback) {
            this();
        }
