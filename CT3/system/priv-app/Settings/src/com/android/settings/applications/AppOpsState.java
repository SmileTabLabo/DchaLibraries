...
public class AppOpsState {
    ...
    public static final Comparator<AppOpEntry> RECENCY_COMPARATOR = new Comparator<AppOpEntry>() { // from class: com.android.settings.applications.AppOpsState.1
    ...
    public static final Comparator<AppOpEntry> LABEL_COMPARATOR = new Comparator<AppOpEntry>() { // from class: com.android.settings.applications.AppOpsState.2
        private final Collator sCollator = Collator.getInstance();
    ...
    private AppEntry getAppEntry(Context context, HashMap<String, AppEntry> appEntries, String packageName, ApplicationInfo appInfo) {
        ...
        if (appEntry == null) {
            ...
            AppEntry appEntry2 = new AppEntry(this, appInfo);
            appEntry2.loadLabel(context);
            appEntries.put(packageName, appEntry2);
            return appEntry2;
        }
        return appEntry;
    }
                        ...
                        if (appInfo.requestedPermissionsFlags == null || (appInfo.requestedPermissionsFlags[j2] & 2) != 0) {
                            for (int k = 0; k < perms.size(); k++) {
                                if (perms.get(k).equals(appInfo.requestedPermissions[j2]) && !appEntry2.hasOp(permOps.get(k).intValue())) {
                                    ...
                                    AppOpsManager.OpEntry opEntry2 = new AppOpsManager.OpEntry(permOps.get(k).intValue(), 0, 0L, 0L, 0, -1, (String) null);
