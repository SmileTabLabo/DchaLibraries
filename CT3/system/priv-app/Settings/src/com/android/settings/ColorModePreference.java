...
public class ColorModePreference extends SwitchPreference implements DisplayManager.DisplayListener {
    ...
    public void updateCurrentAndSupported() {
        ...
        for (int i = 0; i < transforms.length; i++) {
            if (transforms[i] != -1 && i != 1) {
                ColorTransformDescription desc = new ColorTransformDescription(null);
        ...
        for (int i2 = 0; i2 < supportedColorTransforms.length; i2++) {
            ...
            while (true) {
                if (j >= this.mDescriptions.size()) {
                    break;
                } else if (this.mDescriptions.get(j).colorTransform != supportedColorTransforms[i2].getColorTransform() || this.mDescriptions.get(j).transform != null) {
                    j++;
                } else {
    ...
    public static class ColorTransformDescription {
        ...
        /* synthetic */ ColorTransformDescription(ColorTransformDescription colorTransformDescription) {
            this();
        }
