...
public final class BluetoothPermissionRequest extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        switch (this.mRequestType) {
            case 2:
                ...
                message = context.getString(R.string.bluetooth_pb_acceptance_dialog_text, aliasName, aliasName);
                break;
            case 3:
                title = context.getString(R.string.bluetooth_map_request);
                message = context.getString(R.string.bluetooth_map_acceptance_dialog_text, aliasName, aliasName);
                break;
            case 4:
                title = context.getString(R.string.bluetooth_sap_request);
                message = context.getString(R.string.bluetooth_sap_acceptance_dialog_text, aliasName, aliasName);
                break;
            default:
                title = context.getString(R.string.bluetooth_connection_permission_request);
                message = context.getString(R.string.bluetooth_connection_dialog_text, aliasName, aliasName);
                break;
        }
        Notification notification = new Notification.Builder(context).setContentTitle(title).setTicker(message).setContentText(message).setSmallIcon(17301632).setAutoCancel(true).setPriority(2).setOnlyAlertOnce(false).setDefaults(-1).setContentIntent(PendingIntent.getActivity(context, 0, connectionAccessIntent, 0)).setDeleteIntent(PendingIntent.getBroadcast(context, 0, deleteIntent, 0)).setColor(context.getColor(17170521)).build();
        notification.flags |= 32;
        NotificationManager notificationManager = (NotificationManager) context.getSystemService("notification");
        notificationManager.notify(getNotificationTag(this.mRequestType), 17301632, notification);
        wakeLock.release();
    }
    ...
    private boolean checkUserChoice() {
        ...
        } else if (this.mRequestType != 4 || (simPermission = cachedDevice.getSimPermissionChoice()) == 0) {
            return false;
        } else {
            if (simPermission == 1) {
                sendReplyIntentToReceiver(true);
