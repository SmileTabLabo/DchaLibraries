...
public class ConfigDialogFragment extends DialogFragment implements DialogInterface.OnClickListener {
            ...
            if (!dialog.isEditing() && !VpnUtils.isVpnLockdown(profile.key)) {
                try {
                    if (lockdownKey == null) {
                        ...
                        Toast.makeText(getActivity(), (int) R.string.lockdown_vpn_already_connected, 1).show();
                    }
                ...
                if (TextUtils.equals(selectedKey, lockdownKey) && !profile.isValidLockdownProfile()) {
                    ...
                    Toast.makeText(getActivity(), (int) R.string.vpn_lockdown_config_error, 1).show();
                }
            }
        } else if (button == -3) {
            disconnect(profile);
    ...
    private void updateLockdownVpn(boolean isVpnAlwaysOn, VpnProfile profile) {
        if (isVpnAlwaysOn) {
            if (!profile.isValidLockdownProfile()) {
                Toast.makeText(getContext(), (int) R.string.vpn_lockdown_config_error, 1).show();
                return;
            }
    ...
    private void connect(VpnProfile profile) throws RemoteException {
        try {
            ...
            Toast.makeText(getActivity(), (int) R.string.vpn_no_network, 1).show();
        }
    }
