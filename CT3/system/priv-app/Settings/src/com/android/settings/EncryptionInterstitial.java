...
public class EncryptionInterstitial extends SettingsActivity {
    ...
    public static Intent createStartIntent(Context ctx, int quality, boolean requirePasswordDefault, Intent unlockMethodIntent) {
        return new Intent(ctx, EncryptionInterstitial.class).putExtra("extra_password_quality", quality).putExtra(":settings:show_fragment_title_resid", R.string.encryption_interstitial_header).putExtra("extra_require_password", requirePasswordDefault).putExtra("extra_unlock_method_intent", unlockMethodIntent);
    }
        ...
        public Dialog onCreateDialog(int dialogId) {
            ...
            switch (dialogId) {
                case 1:
                    ...
                    String exampleAccessibility = list.isEmpty() ? "" : list.get(0).getResolveInfo().loadLabel(getPackageManager());
                    return new AlertDialog.Builder(getActivity()).setTitle(titleId).setMessage(getString(messageId, new Object[]{exampleAccessibility})).setCancelable(true).setPositiveButton(17039370, this).setNegativeButton(17039360, this).create();
                default:
