...
public class BatteryHistoryChart extends View {
    ...
    public BatteryHistoryChart(Context context, AttributeSet attrs) {
        ...
        this.mBatteryWarnLevel = this.mContext.getResources().getInteger(17694800);
        this.mBatteryCriticalLevel = this.mContext.getResources().getInteger(17694798);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
            ...
            while (true) {
                ...
                if (this.mStats.getNextHistoryLocked(rec) && i2 < N) {
                    if (rec.isDeltaData()) {
                        ...
                        x2 = this.mLevelLeft + ((int) (((curWalltime - walltimeStart) * levelWidth) / walltimeChange));
                        if (x2 < 0) {
                            x2 = 0;
                        }
        ...
        if (lastY < 0 || lastX < 0) {
            ...
            x = this.mLevelLeft + ((int) (((this.mEndDataWallTime - walltimeStart) * levelWidth) / walltimeChange));
            if (x < 0) {
                x = 0;
            }
        }
    ...
    void addTimeLabel(Calendar cal, int levelLeft, int levelRight, boolean is24hr) {
        ...
        this.mTimeLabels.add(new TimeLabel(this.mTextPaint, ((int) (((cal.getTimeInMillis() - walltimeStart) * (levelRight - levelLeft)) / walltimeChange)) + levelLeft, cal, is24hr));
    }

    void addDateLabel(Calendar cal, int levelLeft, int levelRight, boolean isDayFirst) {
        long walltimeStart = this.mStartWallTime;
        long walltimeChange = this.mEndWallTime - walltimeStart;
        this.mDateLabels.add(new DateLabel(this.mTextPaint, ((int) (((cal.getTimeInMillis() - walltimeStart) * (levelRight - levelLeft)) / walltimeChange)) + levelLeft, cal, isDayFirst));
    }
