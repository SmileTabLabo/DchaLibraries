...
public class SecuritySettings extends SettingsPreferenceFragment implements Preference.OnPreferenceChangeListener, DialogInterface.OnClickListener, Indexable, GearPreference.OnGearClickListener {
    ...
    private Runnable mScrollRunner = new Runnable() { // from class: com.android.settings.SecuritySettings.1
    ...
    private void warnAppInstallation() {
        this.mWarnInstallApps = new AlertDialog.Builder(getActivity()).setTitle(getResources().getString(R.string.error_title)).setIcon(17301543).setMessage(getResources().getString(R.string.install_all_warning)).setPositiveButton(17039379, this).setNegativeButton(17039369, this).show();
    }
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        ...
        if ("unlock_set_or_change".equals(key)) {
            ...
            return true;
        } else if ("unlock_set_or_change_profile".equals(key)) {
    ...
    public boolean onPreferenceChange(Preference preference, Object value) {
        ...
        if ("visiblepattern_profile".equals(key)) {
            ...
            if (((Boolean) value).booleanValue()) {
                boolean compliantForDevice = this.mLockPatternUtils.getKeyguardStoredPasswordQuality(this.mProfileChallengeUserId) >= 65536 ? this.mLockPatternUtils.isSeparateProfileChallengeAllowedToUnify(this.mProfileChallengeUserId) : false;
                UnificationConfirmationDialog dialog = UnificationConfirmationDialog.newIntance(compliantForDevice);
                dialog.show(getChildFragmentManager(), "unification_dialog");
                return true;
            }
            String title = getActivity().getString(R.string.unlock_set_unlock_launch_picker_title);
            ChooseLockSettingsHelper helper = new ChooseLockSettingsHelper(getActivity(), this);
            if (helper.launchConfirmationActivity(130, title, true, MY_USER_ID)) {
                return true;
            }
            ununifyLocks();
            return true;
