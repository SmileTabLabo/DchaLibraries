...
public class ActionBar extends RelativeLayout {
    ...
    View getViewAt(int rowIdx, int colIdx) {
        if (rowIdx < 0 || rowIdx > this.mRowsContainer.getChildCount()) {
            throw new IllegalArgumentException(String.format(null, "Row index out of range (requested: %d, max: %d)", Integer.valueOf(rowIdx), Integer.valueOf(this.mRowsContainer.getChildCount())));
        }
        if (colIdx < 0 || colIdx > this.mNumColumns) {
            throw new IllegalArgumentException(String.format(null, "Column index out of range (requested: %d, max: %d)", Integer.valueOf(colIdx), Integer.valueOf(this.mNumColumns)));
        }
