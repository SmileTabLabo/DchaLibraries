...
public class CompatPinnedList {
    ...
    private void setPins(Uri uri, Set<String> pins) {
        ...
        edit.putStringSet("pinned_" + uri.toString(), pins).commit();
    }
    ...
    private void setSpecs(Uri uri, ArraySet<SliceSpec> specs) {
        ...
        SharedPreferences.Editor edit = getPrefs().edit();
        SharedPreferences.Editor putString = edit.putString("spec_names_" + uri.toString(), TextUtils.join(",", specNames));
        putString.putString("spec_revs_" + uri.toString(), TextUtils.join(",", specRevs)).commit();
    }
